...
function ConsistentHash(options) {
     ...
     * can't be changed.
     */
    self.vnodes = options.vnodes || options.topology.vnodes || 100000;
    ...
    if (options.topology) {
        ...
        log.debug({topology: topology}, 'ConsistentHash.new: previous topology');
        self.pnodeToVnodeMap_ = topology.pnodeToVnodeMap;
        var pnodeKeys = Object.keys(self.pnodeToVnodeMap_);

        for (var i = 0; i < pnodeKeys.length; i++) {
            var pnode = pnodeKeys[i];
...
ConsistentHash.prototype.remapVnode = function remapVnode(pnode, vnodes, cb) {
    ...
    self.emit('update', self.serialize(), changedNodes);
    if (cb) {
        return cb(null, self.serialize(), changedNodes);
    }
...
ConsistentHash.prototype.removePnode = function removePnode(pnode, cb) {
    ...
    self.emit('update', self.serialize(), changedMapping);
    if (cb) {
        return cb(null, self.serialize(), pnode);
    }
