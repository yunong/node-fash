...
function ConsistentHash(options) {
    ...
    /**
     * The map of {hashspace -> vnode, ...}, used for quick lookups for
    ...
    if (options.topology) {
        ...
        for (var j = 0; j < self.vnodes; j++) {
            var hashspace = self.vnodeHashInterval_.mul(j);
            log.debug({
                hashspace: hashspace.toString(),
                hashspace_hex: hashspace.toString(16),
                vnode: j
            }, 'ConsistentHash.new: mapping vnode to hashspace');
            self.vnodeToHashspaceMap_[j] = hashspace.toString(16);
            self.hashspaceToVnodeMap_[hashspace.toString(16)] = j;
        }
...
ConsistentHash.prototype.remapNode = function addNode(pnode, vnodes, cb) {
    ...
    for (var i = 0; i < vnodes.length; i++) {
        ...
        var hashspace = self.vnodeToHashspaceMap_[vnode];
        var ringIdx = self.binarySearch(self.ring_, bignum(hashspace, 16));
    ...
    log.debug('updatedRing', self.ring_);
...
ConsistentHash.prototype.getNode = function getNode(key) {
    ...
    var ringIndex = self.findVnode(valueNum);
...
ConsistentHash.prototype.serialize = function serialize() {
    ...
    // deep copy the ring
    var ring = [];
    for (var i = 0; i < self.ring_.length; i++) {
