 ...
 * @param {Function} cb The optional callback function.
 */
    ...
    if (vnodes) {
        vnodes.forEach(function(v) {
            if ((v >= self.vnodeCount_) || (v < 0)) {
                throw new Error('vnode ' + v + ' does not exist in the ring');
            }
            if (vnodeMap[v]) {
...
ConsistentHash.prototype.removePnode = function removePnode(pnode, cb) {
    ...
    var vnodes = self.pnodeToVnodeMap_[pnode];
    if (!vnodes) {
        log.warn({pnode: pnode},
            'ConistentHash.removePnode: pnode not in ring, skipping');
        return cb();
    }
...
ConsistentHash.prototype.getNode = function getNode(key) {
    var self = this;
    var value = crypto.createHash(self.algorithm_.algorithm).update(key).digest('hex');
    // find the node that corresponds to this hash.
    var vnode = self.findVnode(value).toString();
    var pnode = self.vnodeToPnodeMap_[vnode].pnode;
    var data = self.pnodeToVnodeMap_[pnode][vnode];
