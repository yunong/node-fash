...
function ConsistentHash(options) {
        ...
        assert.number(options.numberOfReplicas, 'options.numberOfReplicas');
        ...
        * keeps track of the hashspace that each physical node owns. Used for
        * quick lookups on add/remove node
        */
        this.nodeMap = {};

        /**
        * The map of {hashspace -> node, ...}, used for quick lookups for
        * collision detection and node removal.
        */
        this.ringMap= {};

        /**
        * The sorted array of [{hashSpace, node, replica}, ... ] that
        * represents the hash ring. This can be used to persist the topology of
        * the hash ring and
        ...
        */
        ...
        if (typeof(options.nodes[0]) === 'string') {
                ...
                * 1) Hash each node in order. The resulting hash is the node's
                * place on the ring.
                * 2) If there is a collision, then hash(hash) until no
                ...
                */
                ...
                // clone the topology
                options.nodes.forEach(function(node, index) {
                        // parse the hashspace, since it's serialized as a
                        // string
...
ConsistentHash.prototype.getNode = function getNode(key) {
        ...
        log.trace('key %s hashed to value %s hex, %s dec', key, value,
                  valueNum);
...
ConsistentHash.prototype.removeNode = function removeNode(node) {
        ...
        if (!nodeMap[node]) {
                log.trace('node %s doesn\'t exist, silently returning', node);
                return;
        }
...
function hashAndAdd(self, node, numberOfReplicas) {
        ...
        for (var i = 0; i < numberOfReplicas; i++) {
                ...
                log.trace('pushing into ring',
                          {hashSpace: bignum(hashSpace, 16), node: node});
