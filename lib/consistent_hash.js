...
function ConsistentHash(options) {
    ...
    self.vnodeHashInterval_ = self.algorithmMax_.div(self.vnodesBignum);

    /**
     * The String array of physical nodes in the ring.
     */
    self.pnodes = options.pnodes ? options.pnodes.slice() : [];
    self.pnodes.sort();
    ...
    if (options.topology) {
        ...
        self.pnodes.forEach(function(pnode, index) {
            ...
            pnodeMap[pnode] = true;
            self.pnodeToVnodeMap_[self.pnodes[index]] = {};
        });

        // Allocate the vnodes to the pnodes by
        // vnode % total_pnode = assigned pnode.
        function allocateVnode() {
            for (var vnode = 0; vnode < self.vnodes; vnode++) {
                var pnode = self.pnodes[vnode % self.pnodes.length];
