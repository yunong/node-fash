...
function ConsistentHash(options) {
    ...
    function createNewRing() {
         ...
         * 2) create /vnode/n keys which map vnodes to pnodes. The values is
         * the pnode.
         * 3) create /pnode/<pnode>/<vnode> keys which map pnode to
         * vnodes. The value is the data of the vnode, defaults to 1 since
         * leveldb requires a value.
         * 4) create /pnode/<pnode> keys for all pnodes. The value is the set
         * of all vnodes that belong to this pnode.
        ...
        // 1)
        var batch = self.db_.batch().put('allvnodes', self.vnodeCount_);
        // 2) 3)
        ...
        allocateVnode();
        // 4)
        function allocateAllPnode() {
            var pnodeMap = {};
            for (var i = 0; i < self.pnodes_.length; i++) {
                var pnode = self.pnodes_[i];
                self.db_.put(sprintf('/pnode/%s', pnode), 1);
                if (pnodeMap[pnode]) {
        ...
        // 5)
        self.db_.put('algorithm', JSON.stringify(self.algorithm_));
        // 6)
        self.db_.put('version', fash.VERSION);
        self.db_.put('complete', 1);
    }
    if (options.topology) {
