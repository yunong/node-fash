    ...
    function loadFromDb(callback) {
        vasync.pipeline({funcs: [
            ...
            function checkComplete(_, _cb) {
                self.db_.get('complete', function(err, value) {
                    if (err) {
            ...
            function checkVersion(_, _cb) {
                _cb = once(_cb);
                self.db_.get('version', function(err, version) {
                    if (err) {
            ...
            function checkVnodes(_, _cb) {
                self.db_.get('vnodeCount', function(err, vnodeCount) {
                    if (err) {
                        err = new verror.VError(err);
                    }
            ...
            function checkSlashVnodeSlashN(_, _cb) {
                ...
                self.db_.get(sprintf('/vnode/%d', _.vnodeCount - 1),
                             function(err, pnode) {
                    if (err) {
                        err = new verror.VError(err);
                    }
