...
function ConsistentHash(options, cb) {
    ...
    function createNewRing(callback) {
        ...
        self.algorithm_.vnodeHashInterval_ =
        ...
        var tasks = [
            ...
            function allocateVnodes(_, cb) {
                ...
                for (var vnode = 0; vnode < self.vnodeCount_; vnode++) {
                    ...
                    var hashspace = common.findHashspace({
                        vnode: vnode,
                        vnodeHashInterval: self.algorithm_.vnodeHashInterval_,
                        log: log
                    });
            ...
            function writeMetadata(_, cb) {
                ...
                algorithm.vnodeHashInterval_ =
                    self.algorithm_.vnodeHashInterval_.toString(16);
    ...
    function deserialize(callback) {
        ...
        vasync.pipeline({funcs: tasks = [
            ...
            function writeMetadata(_, cb) {
                ...
                _.batch = _.batch.put('algorithm', topology.algorithm);
    ...
    function loadFromDb(callback) {
        levelup(options.location, {
            ...
            if (err) {
                return callback (new verror.VError(err, 'unable to load db ' +
                                                   'from disk'));
            } else {
                self.db_ = db;
                log.info('successfully loaded ring from disk');
                return callback();
            }
        });
    };
...
ConsistentHash.prototype.getNode = function getNode(key, callback) {
    ...
    var vnode = common.findVnode({
        hash: value, vnodeHashInterval: self.algorithm_.vnodeHashInterval_});
        ...
        function getVnodeToPnodeMaps(_, cb) {
            ...
            for (var vnode = 0; vnode < serializedHash.vnodes; vnode++) {
                db.get(sprintf('/vnode/%d', vnode),
                       (function(vnode, err, pnode)
                {
                    if (err) {
                        return cb(new verror.VError(err));
                    }
                    serializedHash.pnodeToVnodeMap[pnode] = {};
                    db.get(sprintf('/pnode/%s/%d', pnode, vnode),
                           function(err, data)
                    {
                        if (err) {
                            return cb(new verror.VError(err));
